...
from mgs.sensors import Camera
...
class GelSightMini(Camera):
    ...
    def __init__(self, args, model: mj.MjModel, data: mj.MjData, cam_name: str) -> None:
        ...
        bg_path = os.path.join(current_dir, "assets", "background_gelsight2017.jpg")
        self._background_img = cv2.imread(bg_path)

        if self._background_img is None:
            print(f"[WARNING] Background image not found in: {bg_path}")
        else:
            self._background_img = cv2.resize(
                self._background_img, 
                (self._args.cam_width, self._args.cam_height)
            )
